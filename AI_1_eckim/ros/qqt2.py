...
def main(args=None):
    try:
        ...
        while True:
            ...
            gesture = socket_server.receive_data()
            if gesture:
                print(f"Received from client:{gesture}")
            
            #타이머 start
            if gesture == b'Closed_Fist':
                if gesture:
                    fist_timer.timer_init()
                else:
                    fist_timer.timer_reset()
            
            elif gesture == b'Open_Palm':
                if gesture:
                    palm_timer.timer_init()
                else:
                    palm_timer.timer_reset()
