...
def run_mainstream(use_popup=True):
    ...
    try:
        while True:
            ...
            closest_hand = showHandGesture(recognizer, color_image, depth_image) #showHandGesture 함수는 손에 랜드마크를 그려주고, hand gesture의 결과를 반환함, 여기서 gesture 시작
            ...
            if track_victory_gesture():
                mean_rgb, mean_hsv = mean_color(color_image, color_ROI[hand_person_index])
                print(mean_rgb)
                print(mean_hsv)
