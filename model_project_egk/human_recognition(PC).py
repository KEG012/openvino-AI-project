...
def showHandGesture(recognizer,color_image, depth_img, results):   
    # BGR 이미지를 RGB로 변환
    ...
    draw_hand_landmarks(color_image, hand_results)
    
    closest_hand = find_closest_hand(hand_results, depth_img)
    
    global tracked_person_id
    if closest_hand:
        person_idx = match_hand_to_person(closest_hand, results)
        if person_idx is not None and tracked_person_id is not None:
            tracked_person_id = person_idx
...
def match_hand_to_person(hand_position, results):
    x, y, _ = hand_position
...
def find_nearest_person(x, y, boxes):
    min_distance = float('inf')
    ...
    for idx, box in enumerate(boxes):
        ...
        if distance < min_distance:
            ...
            nearest_person_idx = idx

    return nearest_person_idx
    ...
    for idx, result in enumerate(results):
        try:
            ...
            for idx, box in enumerate(boxes):
                ...
                # 사람이 제스처 수행자라면 ID를 1로 고정
                if tracked_person_id == idx:
                    label = f"Person {1}"
                else:
                ...
                cv2.putText(color_img, label,
                            (x1, y1 - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                            (0,255,0), 2, cv2.LINE_AA)
                
                if recognized_gesture != 'No Gesture' and keypoints[0][0].shape[0] >= 2:
                    hand_x, hand_y = keypoints[0][0][:2]
                    if hand_x is not None and hand_y is not None:
                        nearest_idx = find_nearest_person(hand_x, hand_y, boxes)

                        if nearest_idx is not None and tracked_person_id is not None:
                            tracked_person_id = nearest_idx
                        else:
                            print("No valid person found for tracking.")
                    else:
                        print("Hand coordinates are missing, skipping tracking.")
                else:
                    print("No gesture detected or insufficient keypoints.")
...
def run_extract_pose(use_popup):
    ...
    try:
        while True:
            ...
            showHandGesture(recognizer, flipped_color_image, flipped_depth_image, results)
