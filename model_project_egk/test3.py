...
rgb_data = "RGB Color : None"
hsv_data = "HSV ColorNone"
...
def run_mainstream(use_popup=True):
    ...
    try:
        while True:
            ...
            print(z)
            closest_hand = showHandGesture(recognizer, color_image, depth_image)
            ...
            if track_victory_gesture():
                ...
                print(mean_rgb)
                print(mean_hsv)
                rgb_data = f"rgb_color : {mean_rgb}"
                hsv_data = f"hsv_color : {mean_hsv}"

            cv2.putText(color_image, rgb_data, (360, 430), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2, cv2.LINE_AA)
            cv2.putText(color_image, hsv_data, (360, 460), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2, cv2.LINE_AA)
