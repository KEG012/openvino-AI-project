...
def get_keypoint_depths(depth_image, pose, point_score_threshold):
    ...
    keypoints = [left_ear_idx, right_ear_idx,
...
def calculate_body_distances(keypoint_depths):
    ...
    if left_ear_idx in keypoint_dict and left_shoulder_idx in keypoint_dict:
        distances['left_ear_to_left_shoulder'] = calculate_distance(keypoint_dict[left_ear_idx], keypoint_dict[left_shoulder_idx])
    
    if right_ear_idx in keypoint_dict and right_shoulder_idx in keypoint_dict:
        distances['right_ear_to_right_shoulder'] = calculate_distance(keypoint_dict[right_ear_idx], keypoint_dict[right_shoulder_idx])
    
    if left_ear_idx in keypoint_dict and left_hip_idx in keypoint_dict:
        distances['left_ear_to_left_hip'] = calculate_distance(keypoint_dict[left_ear_idx], keypoint_dict[left_hip_idx])
    
    if right_ear_idx in keypoint_dict and right_hip_idx in keypoint_dict:
        distances['right_ear_to_right_hip'] = calculate_distance(keypoint_dict[right_ear_idx], keypoint_dict[right_hip_idx])
